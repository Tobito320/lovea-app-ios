...
## Change after implementation (25.09., Ahmed decided)

The repo is public at times, so the 1,324 ExerciseDB GIFs are NOT committed or bundled. Instead:
- `Lovea/Sources/Health/uebungen.json` (catalog only) is a normal bundle resource; no `Lovea/Uebungen` folder reference, `project.yml` unchanged.
- `UebungsMedien` (in `Uebungen.swift`) fetches `https://static.exercisedb.dev/media/<id>.gif` once into Caches; `UebungGif` (in `UebungsSuche.swift`) shows it with a loading or offline state; `UebungVorschau` uses the same file.
- `TrainingKarte` prefetches the own plan's GIFs (`.task(id: plan)`), so they play offline in the gym.
- `tools/uebungen-laden.sh` downloads into git-ignored `tools/gifs/`, only so `tools/uebungen-katalog.sh` keeps the exercises that have a GIF (176 of 1,500 have none).
- Wherever the code below says `UebungsKatalog.gif(...)` or `Lovea/Uebungen`, the committed code uses the above instead.
...
Result (done): ExerciseDB has no GIF for 176 of the 1,500 ids (HTTP 404), so the catalog keeps the 1,324 with a GIF (127 MB); `uebungen-katalog.sh` filters on the GIFs present. The download already ran (`tools/uebungen-laden.sh`), and a translation agent writes `tools/uebungen-namen-de.json`. The controller then:
...
final class UebungsKatalogTests: XCTestCase {
    ...
    func testBundledCatalogIsCompleteAndSearchable() throws {
        ...
        for u in alle { XCTAssertNotNil(UebungsKatalog.gif(u.id, bundle: bundle), u.id) }
