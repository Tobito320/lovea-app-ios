...
private struct ProfilInhalt: View {
    ...
    /// Coordination note (brief-F/E): reads `PunkteModell.shared.stand[person]` directly, the
    /// lifetime points earned — the same number Health/E's chip reads, for a consistent figure
    /// across the app. That's a DIFFERENT, larger number than the Shop's spendable balance
    /// (`PunkteModell.einkaufsStand`, which subtracts what's already been spent) — see ShopView.
    private var punkteChip: some View {
        let punkte = PunkteModell.shared.stand[person] ?? 0
        return chip("✨", "\(punkte) Punkte", "\(punkte) Punkte")
    }
